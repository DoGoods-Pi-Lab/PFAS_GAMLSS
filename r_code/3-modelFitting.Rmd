...
for(i in 1:ncol(exp)){
    ...
    ##main model next  
        mainDesign<-model.matrix(~exp[,i] + myPheno$PC1 + myPheno$PC2 + myPheno$PC3 +
    ...
    ##complex model last  
        complexDesign<-model.matrix(~exp[,i] + myPheno$PC1 + myPheno$PC2 + myPheno$PC3 +
                                  myPheno$Parity + myPheno$race_B + myPheno$race_NBNW + myPheno$smoke_Preg + myPheno$smoke_unknown +
                                  myPheno$sex +
        ...
        #make your summary-strict
        #modelsummary<-summary(decideTests(ebfit, method="separate", adjust.method="BH", p.value=9e-8,lfc=0))
        #mal_modelsum_strict<-rbind(mal_modelsum_strict, modelsummary)
        
        #pull out a few things - all genes, the significant genes, and then the top 100 genes
        #genes.model<-output$UCSC_RefGene_Name #This variable is weird - I make a copy of it with only one gene name per row
        #genes.model <- genes.model[!is.na(genes.model)] ##First, assign NA to blank cells
        #is.na(genes.model)<-which(genes.model == "")
        #genefirst<-sapply(strsplit(as.character(genes.model), "\\;"), "[[", 1) ##Now pull out the first one from the broken up list
        #model<-cbind(output,genefirst)
        ##write.csv(model, file = paste0("./2.Results_Genes/AllGenes/MALE",myphth_names[[i]],"_allgenes.csv")) 

        #Identify significant genes and top 100 genes to save for later
        #subsetq<-subset(model,model$adj.P.Val<0.2) 
        #dim(subsetq)
        #subsetq2<-subsetq[order(subsetq$P.Value, decreasing=FALSE),]
        ##write.csv(subsetq2, file=paste0("./2.Results_Genes/SigGenes/MALE",myphth_names[[i]],"significant.csv")) 
        
        #top by pvalue
        #top<-model%>% subset(P.Value<0.001)
        ##write.csv(top, file=paste0("./2.Results_Genes/Sig0.001/MALE",myphth_names[[i]],"_top.csv"))
        
        #Use model design from before - best model - this is commented out b/c there are so few sig sites
        ...
        ###write.csv(DMRs, paste0("./2.Results_Genes/DMR/MALE",myphth_names[[i]],"DMR.csv"), row.names=TRUE)
